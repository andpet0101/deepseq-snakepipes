...
database_specific_hits_plot = ggplot(fastq_screen_result$databases,aes(x=Library,y=perc_One_hit_one_library+perc_Multiple_hits_one_library,fill=Database)) + 
	...
	theme(strip.text.y=element_blank(),legend.position="bottom",axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
	ggtitle("Contamination: reads exclusive to database")

ggsave(paste(pdf_directory,paste(bfx_id,"contamination_database_specific_hits.pdf",sep="_"),sep="/"),plot=database_specific_hits_plot,width=7,height=9)
