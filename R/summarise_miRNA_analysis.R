...
read_status_len_table = read_status_len_table %>% 
			group_by(library) %>% dplyr::mutate(frac_count = count/sum(count)) %>%
			as.data.frame() 
read_status_summary = read_status_len_table %>%
			group_by(library,type) %>% dplyr::summarise(count=sum(count)) %>%
			as.data.frame()
...
read_status_len_table = read_status_len_table %>% 
			...
			as.data.frame() 
read_status_summary = read_status_len_table %>%
			group_by(library,plottype) %>% dplyr::summarise(count=sum(count)) %>%
			as.data.frame()
...
##########################################
# 3. plot filtered genomic mapping rates #
##########################################
...
################################################################################
# 4. read in mirdeep star tables, add library and normalised expression levels #
################################################################################
...
mirdeep_star_data_table = ldply(mirdeep_star_files,function(file){
	mirdeep_table = read.csv(file,header=T,sep="\t",comment.char="")
	mirdeep_table = mirdeep_table %>% group_by(miR_ID) %>% filter(score==max(score)) %>% as.data.frame()
	mirdeep_table$library = gsub('_R\\d$','',gsub('\\.\\S+$','',basename(file)))
	mirdeep_table
})

mirdeep_star_data_table = separate(mirdeep_star_data_table,miR_ID,c("miR_ID","Hairpin_ID","miR_Name"),sep="/",fill="right")

colnames(mirdeep_star_data_table) = c("miR_ID","Hairpin_ID","miR_Name","score","chr","strand","location_precursor","count","location_miRNA","precursor_seq","precursor_sec_struct","mature_seq_precursor_seq","star_loop_seq","UMI_reads","UMIs","UMI_distribution","library")
...
mirdeep_star_data_table$type = factor(ifelse(grepl('novelMiR',mirdeep_star_data_table$miR_ID),"novel","known"),levels=c("known","novel"))
mirdeep_star_data_table = mirdeep_star_data_table[,c("library","miR_ID","Hairpin_ID","miR_Name","type","score","chr","strand","location_precursor","count","location_miRNA","precursor_seq","precursor_sec_struct","mature_seq_precursor_seq","star_loop_seq","UMI_reads","UMIs","UMI_distribution")]

# filter by score (known miRNAs: >=0, novelmiRNAs: >=7)
mirdeep_star_table = subset(mirdeep_star_data_table,(type=="novel" & score>=7) | (type=="known"))

# average counts for the same miRNA coming from different precursors 
mirdeep_star_table = mirdeep_star_table %>%
  group_by(library,miR_ID) %>%
...
mirdeep_star_table = mirdeep_star_table %>%
			group_by(library) %>% dplyr::mutate(norm_count = mean_count*1000000/sum(mean_count)) %>%
			as.data.frame()
...
################################
mature_mirna_summary = mirdeep_star_table[,c("library","type","miR_ID","norm_count")] %>% 
  group_by(library,type,miR_ID) %>%
  dplyr::summarise(norm_count=sum(norm_count)) %>% 
  as.data.frame()
mature_mirna_summary = mature_mirna_summary %>% 
  complete(library,type,miR_ID,fill=list(norm_count=0)) %>%
  group_by(library,type) %>% 
  dplyr::summarise(All=sum(norm_count>0),count_0=sum(norm_count>0 & norm_count<=10),count_10=sum(norm_count>10 & norm_count<=100),count_100=sum(norm_count>100 & norm_count<=1000),count_1000=sum(norm_count>1000 & norm_count<=10000),count_10000=sum(norm_count>10000)) %>%		
			as.data.frame()
			
colnames(mature_mirna_summary) = c("library","type","All","0-10","11-100","101-1000","1001-10000",">10000")
mature_mirna_summary_m = gather(mature_mirna_summary,"metric","value",-c(library,type),factor_key = T)
mature_mirna_summary_m$metric = factor(mature_mirna_summary_m$metric,levels=c("All","0-10","11-100","101-1000","1001-10000",">10000"))
mature_mirna_summary_m$type = relevel(mature_mirna_summary_m$type,"novel")

number_of_profiled_miRNA_plot = ggplot(subset(mature_mirna_summary_m,metric=="All"),aes(x=library,y=value,fill=library,alpha=type)) +
geom_bar(stat="identity",colour="black") +
scale_x_discrete("Library") +
scale_y_continuous("Number of identified mature miRNAs") +
theme_bw() +
...
ggtitle("Identified mature miRNAs")
ggsave(paste(bfx_id,"number_of_profiled_mature_miRNAs.pdf",sep="_"),number_of_profiled_miRNA_plot,path=pdf_directory,width=9,height=8)

miRNA_read_counts_plot = ggplot(subset(mature_mirna_summary_m,metric!="All"),aes(x=library,y=value,fill=library,alpha=type)) +
geom_bar(stat="identity",colour="black") +
scale_x_discrete("Library") +
...
ggtitle("Mature miRNAs by expression level (cpm)")
ggsave(paste(bfx_id,"number_of_profiled_mature_miRNAs_by_expression_level.pdf",sep="_"),miRNA_read_counts_plot,width=9,height=8,path=pdf_directory)

mirRNA_ranked_table =  mirdeep_star_table[,c("library","type","miR_ID","norm_count")] %>% 
  group_by(library,type,miR_ID) %>%
  dplyr::summarise(norm_count=sum(norm_count)) %>% 
  ungroup() %>% 
  group_by(library) %>% 
  dplyr::mutate(rank=rank(-norm_count,ties.method="random")) %>% 
  dplyr::arrange(rank) %>% 
  dplyr::mutate(frac_cum_sum=cumsum(norm_count)/sum(norm_count))
...
geom_line() +
theme_bw() +
scale_x_continuous("Mature miRNAs ranked from high to low expressed",trans=log10_trans(),limits=c(1,max_rank),breaks=c(1,10,50,100,seq(200,round(max_rank,-2),100))) +
scale_y_continuous("Fraction of mature miRNA reads",limits=c(0,1)) +
scale_colour_manual("Library",values=color_brewer_qual_palette) +
theme(legend.position="bottom") +
ggtitle("Mature miRNAs ranked by expression (high to low)")
ggsave(paste(bfx_id,"mature_miRNAs_ranked_by_expression_level_plot.pdf",sep="_"),miRNA_ranked_counts_plot,width=9,height=8,path=pdf_directory)

############################################################
# 8. expression table and correlation plot for known miRNA #
############################################################

mirdeep_star_table_known = subset(mirdeep_star_table,type=="known")
miRNA_expression_table = spread(mirdeep_star_table_known[,c("library","miR_ID","Hairpin_ID","miR_Name","mean_count")],library,mean_count,fill=0)
write.table(miRNA_expression_table,paste(data_directory,paste(bfx_id,"known_mature_miRNAs_raw_counts.csv",sep="_"),sep="/"),quote=F,sep="\t",row.names=F,col.names=T)

miRNA_expression_table = mirdeep_star_table_known[,c("library","miR_Name","norm_count")] %>% 
  group_by(library,miR_Name) %>%
  dplyr::summarise(norm_count=sum(norm_count)) %>% 
  as.data.frame() %>%
  spread(library,norm_count,fill=0)
rownames(miRNA_expression_table) = miRNA_expression_table$miR_Name
miRNA_expression_table$miR_Name = NULL


if(ncol(miRNA_expression_table)>1){
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="spearman", cluster=FALSE, title = "Spearman correlation of counts of known miRNAs",filename=paste(pdf_directory,paste(bfx_id,"known_mature_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="pearson", cluster=FALSE, title = "Pearson correlation of counts of known miRNAs",filename=paste(pdf_directory,paste(bfx_id,"known_mature_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
} else {
	ggsave(grid.text("No known_miRNA_expression_spearman_correlation plot possible"),file=paste(pdf_directory,paste(bfx_id,"known_mature_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	ggsave(grid.text("No known_miRNA_expression_pearson_correlation plot possible"),file=paste(pdf_directory,paste(bfx_id,"known_mature_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
}
...
novel_miRNAs = mirdeep_star_table_novel[,c("library","miR_ID","chr","location_precursor","strand")]
...
novel_miRNAs_count_table = mirdeep_star_table_novel[,c("library","group","mean_count","norm_count")] %>% group_by(library,group) %>% dplyr::summarise(mean_count=mean(mean_count),norm_count=mean(norm_count)) %>% as.data.frame()
colnames(novel_miRNAs_count_table) = c("library","miRNA","mean_count","norm_count")

miRNA_expression_table = spread(novel_miRNAs_count_table[,c("library","miRNA","mean_count")],library,mean_count,fill=0)
write.table(miRNA_expression_table,paste(data_directory,paste(bfx_id,"novel_mature_miRNAs_raw_counts.csv",sep="_"),sep="/"),quote=F,sep="\t",row.names=F,col.names=T)
...
if(ncol(miRNA_expression_table)>1){
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="spearman", cluster=FALSE, title = "Spearman correlation of counts of novel miRNAs", filename=paste(pdf_directory,paste(bfx_id,"novel_mature_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="pearson", cluster=FALSE,  title = "Spearman correlation of counts of novel miRNAs", filename=paste(pdf_directory,paste(bfx_id,"novel_mature_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
} else {
	ggsave(grid.text("No novel_miRNA_expression_spearman_correlation plot possible"),file=paste(pdf_directory,paste(bfx_id,"novel_mature_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	ggsave(grid.text("No novel_miRNA_expression_pearson_correlation plot possible"),file=paste(pdf_directory,paste(bfx_id,"novel_mature_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
}
...
# 	dplyr::mutate(R2 = summary(mod)$r.squared,intercept = coef(mod)[1],slope=coef(mod)[2]) %>% as.data.frame()
