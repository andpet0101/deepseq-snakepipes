...
too_many_datasets = length(unfiltered_mapping_logs) > 20
...
unfiltered_mapping_summary_m$metric = factor(unfiltered_mapping_summary_m$metric,levels=c("reads_aligned_with_multi","reads_unaligned"))

if(!too_many_datasets){
  unfiltered_mapping_summary_plot1 = ggplot(unfiltered_mapping_summary_m,aes(x=library,y=value,fill=metric,group=rev(metric))) +
    geom_bar(stat="identity",position="stack",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Number of reads",labels=comma) +
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
    theme(axis.text.x=element_text(angle=45,vjust = 1, hjust=1)) +
    ggtitle("Mapping numbers of input reads")
}else{
  unfiltered_mapping_summary_plot1 = ggplot(unfiltered_mapping_summary_m,aes(x=metric,y=value,fill=metric,group=metric)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Mapping status",labels=c("aligned","unaligned"),drop=F) +
    scale_y_continuous("Number of reads",labels=comma) +
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
...
if(!too_many_datasets){
  unfiltered_mapping_summary_plot2 = ggplot(unfiltered_mapping_summary_m,aes(x=library,y=value,fill=metric,group=rev(metric))) +
  ...
  plot_data = unfiltered_mapping_summary_m %>% group_by(library) %>% mutate(frac_value=value/sum(value)) %>% as.data.frame()
  unfiltered_mapping_summary_plot2 = ggplot(plot_data,aes(x=metric,y=frac_value,fill=metric,group=metric)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Mapping status",labels=c("aligned","unaligned"),drop=F) +
    scale_y_continuous("Percentage of reads",label=percent) +
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
    theme(legend.position = "none") +
    ggtitle("Mapping rates of input reads")
}
...
if(!too_many_datasets){
  read_status_summary_plot1 = ggplot(read_status_summary,aes(x=library,y=count,fill=type,group=rev(type))) +
    geom_bar(stat="identity",position="stack",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Number of reads",label=comma) +
    scale_fill_manual("Read status",values=color_brewer_qual_palette,drop=F) + 
    theme(axis.text.x=element_text(angle=45,vjust = 1, hjust=1)) +
    ggtitle("Read status numbers")
}else{
  read_status_summary_plot1 = ggplot(read_status_summary,aes(x=type,y=count,fill=type)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Read status",drop=F) +
    scale_y_continuous("Number of reads",label=comma) +
    scale_fill_manual("Read status",values=color_brewer_qual_palette,drop=F) + 
    theme(legend.position = "none") +
    ggtitle("Read status numbers")
...
if(!too_many_datasets){
  read_status_summary_plot2 = ggplot(read_status_summary,aes(x=library,y=count,fill=type,group=rev(type))) +
    geom_bar(stat="identity",position="fill",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Percentage of reads",label=percent) +
  ...
  plot_data = read_status_summary %>% group_by(library) %>% mutate(frac_count=count/sum(count)) %>% as.data.frame()
  read_status_summary_plot2 = ggplot(plot_data,aes(x=type,y=frac_count,fill=type)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Read status",drop=F) +
    scale_y_continuous("Percentage of reads",label=percent) +
...
if(!too_many_datasets){
  read_status_summary_plot3 = ggplot(read_status_summary,aes(x=library,y=count,fill=plottype,group=rev(plottype))) +
    geom_bar(stat="identity",position="stack",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Number of reads",label=comma) +
  ...
  read_status_summary_plot3 = ggplot(read_status_summary,aes(x=plottype,y=count,fill=plottype)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Read status",drop=F) +
    scale_y_continuous("Number of reads",label=comma) +
...
if(!too_many_datasets){
  read_status_summary_plot4 = ggplot(read_status_summary,aes(x=library,y=count,fill=plottype,group=rev(plottype))) +
    geom_bar(stat="identity",position="fill",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Percentage of reads",label=percent) +
  ...
  plot_data = read_status_summary %>% group_by(library) %>% mutate(frac_count=count/sum(count)) %>% as.data.frame()
  read_status_summary_plot4 = ggplot(plot_data,aes(x=plottype,y=frac_count,fill=plottype)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Read status",drop=F) +
    scale_y_continuous("Percentage of reads",label=percent) +
...
if(!too_many_datasets){
  read_status_length_plot = ggplot(read_status_len_table,aes(x=length,y=frac_count,fill=plottype,colour=plottype,group=rev(plottype))) +
    ...
    scale_fill_brewer("Read status",type="qual",palette=2,direction=-1,drop=F) + 
    scale_colour_brewer("Read status",type="qual",palette=2,direction=-1,drop=F) + 
  ...
  read_status_length_plot = ggplot(read_status_len_table,aes(x=length,y=frac_count,fill=plottype,colour=plottype,group=length)) +
    geom_boxplot() +
    theme_bw() +
    scale_x_continuous("Read length in bp",limits=c(min(read_status_len_table$length),max(read_status_len_table$length))) +
    scale_y_continuous("Fraction of reads") +
...
if(!too_many_datasets){
  filtered_mapping_summary_plot1 = ggplot(filtered_mapping_summary_m,aes(x=library,y=value,fill=metric,group=rev(metric))) +
    geom_bar(stat="identity",position="stack",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    ...
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
    theme(axis.text.x=element_text(angle=45,vjust = 1, hjust=1)) +
    ggtitle("Mapping numbers of filtered reads")
}else{
  filtered_mapping_summary_plot1 = ggplot(filtered_mapping_summary_m,aes(x=metric,y=value,fill=metric)) +
    geom_boxplot(notch=T)+
    theme_bw() +
    scale_x_discrete("Mapping status",drop=F,labels=c("aligned","unaligned")) +
    scale_y_continuous("Numbers of reads",label=comma) +
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
    theme(legend.position = "none") +
    ggtitle("Mapping numbers of filtered reads")
...
if(!too_many_datasets){
  filtered_mapping_summary_plot2 = ggplot(filtered_mapping_summary_m,aes(x=library,y=value,fill=metric,group=rev(metric))) +
    geom_bar(stat="identity",position="fill",colour="black") +
    theme_bw() +
    scale_x_discrete("Library") +
    scale_y_continuous("Percentage of reads",label=percent) +
  ...
  plot_data = filtered_mapping_summary_m %>% group_by(library) %>% mutate(frac_value=value/sum(value)) %>% as.data.frame()
  filtered_mapping_summary_plot2 = ggplot(plot_data,aes(x=metric,y=frac_value,fill=metric,group=metric)) +
    geom_boxplot(notch=T) +
    theme_bw() +
    scale_x_discrete("Mapping status",labels=c("aligned","unaligned"),drop=F) +
    scale_y_continuous("Percentage of reads",label=percent) +
    scale_fill_brewer("Mapping status",type="qual",palette=2,labels=c("aligned","unaligned"),drop=F) +
    theme(legend.position = "none") +
    ggtitle("Mapping rates of filtered reads")
}
...
total_reads_used_table = mirdeep_star_data_table %>% group_by(library) %>% summarise(tot_reads = sum(count)) %>% as.data.frame()

if(!too_many_datasets){
  total_reads_used_plot = ggplot(total_reads_used_table,aes(x=library,y=tot_reads,fill=library)) + 
    geom_bar(stat="identity",colour="black") +
    scale_x_discrete("Library") +
    scale_y_continuous("Number of used reads",labels=comma) +
    theme_bw() +
    scale_fill_manual("Library",values=color_brewer_qual_palette) +
    theme(axis.text.x=element_text(angle=45,vjust = 1, hjust=1),legend.position="none")
}else{
  total_reads_used_plot = ggplot(total_reads_used_table,aes(x=1,y=tot_reads)) + 
    geom_boxplot(fill="orange",colour="black",notch=T) +
    scale_x_discrete("Dataset") +
    scale_y_continuous("Number of used reads",labels=comma) +
    theme_bw() +
    theme(axis.text.x=element_text(angle=45,vjust = 1, hjust=1),legend.position="none") +
    ggtitle("Number of reads used for counting")
}
ggsave(paste(bfx_id,"total_number_of_reads_counted.pdf",sep="_"),total_reads_used_plot,path=pdf_directory,width=9,height=8)
...
plot_data = subset(mirdeep_star_summary_m,metric=="All")
plot_data$window = ceiling(as.integer(plot_data$library)/20)
number_of_profiled_miRNA_plot = ggplot(plot_data,aes(x=library,y=value,fill=library,alpha=type,group=rev(type))) +
  geom_bar(stat="identity",colour="black") +
  scale_x_discrete("Library") +
...
ggsave(paste(bfx_id,"number_of_profiled_miRNAs.pdf",sep="_"),number_of_profiled_miRNA_plot,path=pdf_directory,width=8,height=9)

if(!too_many_datasets){
  miRNA_read_counts_plot = ggplot(subset(mirdeep_star_summary_m,metric!="All"),aes(x=library,y=value,fill=library,alpha=type,group=rev(type))) +
    geom_bar(stat="identity",colour="black") +
    scale_x_discrete("Library") +
    scale_y_continuous("Number of identified miRNAs") +
    theme_bw() +
    facet_wrap(~ metric,scales="free_y",ncol=3) +
    scale_fill_manual("Library",values=color_brewer_qual_palette,drop=F) +
    scale_alpha_manual("Type of miRNA",values=c(1,0.2),labels=c("known","putative novel"),drop=F) +
    theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),legend.position="bottom") +
    ggtitle("MiRNAs by expression level")
}else{
  miRNA_read_counts_plot = ggplot(subset(mirdeep_star_summary_m,metric!="All"),aes(x=metric,y=value,fill=metric)) +
    geom_boxplot(notch=T) +
    scale_x_discrete("Normalised expression level") +
    scale_y_continuous("Number of identified miRNAs") +
    theme_bw() +
    scale_fill_manual("Normalised expression level",values=color_brewer_qual_palette) +
    theme(legend.position="none") +
    ggtitle("MiRNAs by expression level")
}
...
if(too_many_datasets){miRNA_ranked_counts_plot = miRNA_ranked_counts_plot + theme(legend.position = "none")}
...
if(ncol(miRNA_expression_table)>1){
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="spearman", cluster=TRUE, title = "Spearman correlation of counts of known miRNAs",filename=paste(pdf_directory,paste(bfx_id,"known_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="pearson", cluster=TRUE, title = "Pearson correlation of counts of known miRNAs",filename=paste(pdf_directory,paste(bfx_id,"known_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
} else {
...
if(ncol(miRNA_expression_table)>1){
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="spearman", cluster=TRUE, title = "Spearman correlation of counts of novel miRNAs", filename=paste(pdf_directory,paste(bfx_id,"novel_miRNA_expression_spearman_correlation.pdf",sep="_"),sep="/"))
	plotCorrelation(miRNA_expression_table, conditions="", cormethod="pearson", cluster=TRUE,  title = "Spearman correlation of counts of novel miRNAs", filename=paste(pdf_directory,paste(bfx_id,"novel_miRNA_expression_pearson_correlation.pdf",sep="_"),sep="/"))
} else {
