...
if(length(arguments)!=2){
	stop("Needs a raw stats file generated by one or more picard tools calls and an output prefix\n", call.=FALSE)
}

raw_stats_file = arguments[1]
output_prefix = arguments[2]

#raw_stats_file = "test.stats"
#output_prefix = "test_out"

# break text files into parts starting with '# picard.analysis.'
records = readChar(raw_stats_file, file.info(raw_stats_file)$size)
records = unlist(strsplit(records,"# picard.analysis."))
records = sapply(records,function(x){
    x = trimws(x)
